...
const BASE_LOCATIONS = [
    { lat: 5.569787, lon: -0.128979, alt: 0 },
    { lat: 5.728479, lon: -0.015486, alt: 0 }
];
...
function updateDronePhysicsAndState(id: string, drone: Drone) {
    // Realistic speed factor tuned to prevent overshooting targets. 1.0x multiplier is approx. 5 mph.
    const DRONE_SPEED_FACTOR = 0.00002 * drone.cruisingSpeed * SIMULATION_SPEED;
    ...
    switch (drone.status) {
        case DroneStatus.INTERCEPTING: {
            ...
            if (drone.isEliminationApproved) {
                ...
                const speedMps = drone.cruisingSpeed * (1 / MPH_TO_MULTIPLIER) * 0.44704;
