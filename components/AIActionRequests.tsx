...
const RequestCard: React.FC<{
    ...
    const allActions = Object.values(AIAction).filter(a => a !== AIAction.COUNTER_COMMAND);
        ...
        <div key={request.id} className={`bg-gray-900 ${styles.bg} p-4 rounded-lg flex flex-col md:flex-row items-center justify-between gap-4 shadow-md ${styles.border}`}>
            <div className="flex-grow w-full">
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-base">
                    <span className="font-semibold text-gray-400 text-right">{t('ai_requests.drone')}</span>
                    <span className="text-cyan-400 font-bold">{getDroneDisplayName(request.droneId)}</span>
            ...
            <div className="flex-shrink-0 flex flex-col items-stretch gap-3 w-full md:w-[320px]">
                <div className="flex items-center gap-3 w-full">
                    <button
                        ...
                        aria-label={`Approve ${formatAction(request.action)} for ${getDroneDisplayName(request.droneId)}`}
                    >
                    ...
                    <button
                        ...
                        aria-label={`Deny ${formatAction(request.action)} for ${getDroneDisplayName(request.droneId)}`}
                    >
                        {t('ai_requests.deny')}
                    </button>
                </div>
